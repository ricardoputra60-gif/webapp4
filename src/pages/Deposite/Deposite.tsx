...
const DepositForm = () => {	
  ...
  useEffect(() => {	
    ...
    if (Cookies.get('token') != null || "") {	
      // Retrieve nomor_rekening and account_id from localStorage	
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      const response = await axios.post(
        'http://api.servernikki.my.id/api/deposite',
        {
      ...
      setJumlahDeposit(''); // Reset the nominal input
      navigate('/');
    } catch (error) {
      console.error('Error making deposit:', error);
      setMessage('Failed to make deposit');
    }
  };
  ...
  return (	
    <div className="absolute top-20 left-5 m-4 w-full">	
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 w-full max-w-md">	
        <h2 className="text-2xl font-semibold mb-4">Bank Deposit</h2>	
